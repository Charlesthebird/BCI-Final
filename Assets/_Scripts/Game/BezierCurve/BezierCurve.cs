...
public class BezierCurve : MonoBehaviour {
    ...
    private void OnDrawGizmos()
    {
        ...
        {
            Gizmos.color = new Color(i / (float)segments.Count, .5f, i, 0.4f);
            Gizmos.DrawSphere(segments[i].startCP.transform.position, .2f);
            Gizmos.color = new Color(i / (float)segments.Count, .5f, i, 0.8f);
    ...
    /// <summary>
    /// THIS IS WHERE THE CURVE FUNCTION IS INPUT
    /// </summary>
    void UpdateSegmentPositions()
    {
        for (int j = 0; j < segments.Count; j++)
        {
            ...
            for (int i = 0; i < interpolationCount; i++)
            {
                ...
                var p1 = seg.startCP.transform.position;
                var p2 = seg.startCP.rightTangentTransform.position;
                var p3 = seg.endCP.leftTangentTransform.position;
                var p4 = seg.endCP.transform.position;
    ...
    //{
        ...
        var prevSegIndex = segments.Count - 2;
        if (prevSegIndex >= 0)
    ...
    public int GetNextSegNum(float dist)
    {
        if (segments == null || segments.Count <= 1) return -1;
        for (int i = 0; i < segments.Count - 1; i++)
        {
            // if i is the right segment, i+1 is the next segment
            if (dist <= segments[i].lengths[interpolationCount - 1])
            {
                return i + 1;
            }
